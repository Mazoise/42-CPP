...
void		FragTrap::vaulthunter_dot_exe(std::string const &target)
{
	...
	{
		std::cout << this->_type << " " << this->_name << " attacks " << target << " with VaultHunter.EXE : " << std::endl;
		std::cout << "    - " << attacks[std::rand() % 5] << std::endl;
		this->setEnergyPoints(this->_energyPoints - 25);
	}
	else
		std::cout << this->_type << " " << this->_name << " doesn't have enough energy to launch VaultHunter.EXE attack." << std::endl;
}
